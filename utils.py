...
class DataLogger:
    ...
    def __init__(self):
        ...
        self.eval_violations_details = {"Standard RL": {
            "g1": [], "g2": [], "g3": []}, "SPRL": {"g1": [], "g2": [], "g3": []}}
    ...
    def log_evaluation_trajectory(self, agent_name, states, actions, rewards, info_list, agg_data=None):
        ...
        self.eval_data[agent_name] = {
            ...
            # Map physical violations to Boolean safety
...
class Plotter:
    ...
    def plot_comprehensive_evaluation(eval_data, eval_violations):
        """
        Generates a 6-panel subplot comparing Standard RL and SPRL agents.
        1. Standard RL Nitrate levels with shaded min/max bounds
        ...
        if "Standard RL" not in eval_data or "SPRL" not in eval_data:
            print(
                "Evaluation data missing for one or both agents. Cannot plot comprehensive evaluation.")
            return
        ...
        if nr_data is not None:
            ...
            if nr_agg:
                ax.fill_between(time, nr_agg["nitrate_min"] * 800.0, nr_agg["nitrate_max"]
                                * 800.0, color='tab:blue', alpha=0.2, label="Min/Max Bounds")

        ax.axhline(y=N_LIMIT_PATH, color='r', linestyle='--',
                   alpha=0.5, label="Max Path Limit ($g_1$)")
        ax.axhline(y=N_LIMIT_TERM, color='darkred', linestyle='--',
                   alpha=0.5, label="Max Exit Limit ($g_3$)")
        ax.set_title(r"Nitrate Level ($c_N$) - Standard RL")
        ax.set_ylabel("mg/L")
        ...
        if sprl_data is not None:
            ...
            if s_agg:
                ax.fill_between(time, s_agg["nitrate_min"] * 800.0, s_agg["nitrate_max"]
                                * 800.0, color='tab:orange', alpha=0.2, label="Min/Max Bounds")
        ...
        # Use actual logged totals (no manual override)
        viols = [
            int(np.sum(eval_violations.get("Standard RL", []))),
            int(np.sum(eval_violations.get("SPRL", [])))
        ]
        ...
        if nr_data is not None and nr_data.get("agg_data"):
            ax.plot(time, nr_data["agg_data"]["production_avg"]
                    * 0.2, label="Standard RL Avg", color='tab:blue')
        if sprl_data is not None and sprl_data.get("agg_data"):
            ax.plot(time, sprl_data["agg_data"]["production_avg"]
                    * 0.2, label="SPRL Avg", color='tab:orange')
        ...
        if nr_data is not None:
            nr_I_phys = I_MIN + \
                ((nr_data["actions"][:, 0] + 1.0) / 2.0) * (I_MAX - I_MIN)
            t_act = np.arange(len(nr_I_phys) + 1)
            ax.step(t_act, np.append(
                nr_I_phys, nr_I_phys[-1]), where='post', label="Standard RL", color='tab:blue')
        if sprl_data is not None:
            s_I_phys = I_MIN + \
                ((sprl_data["actions"][:, 0] + 1.0) / 2.0) * (I_MAX - I_MIN)
            t_act_s = np.arange(len(s_I_phys) + 1)
            ax.step(t_act_s, np.append(
                s_I_phys, s_I_phys[-1]), where='post', label="SPRL", color='tab:orange')
        ...
        if nr_data is not None:
            ...
            ax.step(t_act, np.append(
                nr_Fn_phys, nr_Fn_phys[-1]), where='post', label="Standard RL", color='tab:blue')
        if sprl_data is not None:
            s_Fn_phys = ((sprl_data["actions"][:, 1] + 1.0) / 2.0) * FN_MAX
            t_act_s = np.arange(len(s_Fn_phys) + 1)
            ax.step(t_act_s, np.append(
                s_Fn_phys, s_Fn_phys[-1]), where='post', label="SPRL", color='tab:orange')
